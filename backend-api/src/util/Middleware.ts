...
export async function isLoginMiddleWare(req:Request,res:Response,next:NextFunction){
...
export async function isPropertyCreator(req:Request,res:Response,next:NextFunction){
    const user = req.user;
    if(!user){
        return next(new Error("User not Logged in"))
    }
    ...
    if(property==null)
        return next(new Error("Property not vaild"));
